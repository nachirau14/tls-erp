...
def show_app():
    ...
    with st.sidebar:
        ...
        st.divider()
        page = st.radio("Nav", ["🏠 Dashboard","📁 Projects","⏱ Log Time","📋 My Time Logs"],
    ...
    elif "Projects" in page: page_projects()
    elif "Log Time" in page: page_log_time(user)
    elif "My Time" in page: page_my_logs(user)

def page_dashboard(user):
    try:
        projects = api.list_projects(); logs = api.list_time_logs()
        emp_id = user.get("employee_id", "")
        my = [l for l in logs if str(l.get("employee_id")) == str(emp_id)] if emp_id else []
    ...
    month_h = sum(l.get("hours", 0) for l in my if l.get("date", "").startswith(this_m))
    today_s = datetime.now().strftime("%Y-%m-%d")
    today_h = sum(l.get("hours", 0) for l in my if l.get("date") == today_s)

    c1,c2,c3,c4 = st.columns(4)
    for c,lbl,v in [(c1,"Active Projects",active),(c2,"Total Hours",f"{total_h:.1f}h"),
                    (c3,"This Month",f"{month_h:.1f}h"),(c4,"Today",f"{today_h:.1f}h")]:
        with c: st.markdown(f'<div class="stat-card"><div class="label">{lbl}</div><div class="value">{v}</div></div>', unsafe_allow_html=True)
...
def page_projects():
    ...
    with st.expander("➕ Create New Project"):
        with st.form("new_proj"):
            a,b = st.columns(2)
            with a: name = st.text_input("Project Name *"); client = st.text_input("Client Name")
            with b:
                cost = st.number_input("Total Receivable (₹)", min_value=0.0, step=10000.0, format="%.2f")
            ...
            if st.form_submit_button("Create Project", type="primary", use_container_width=True):
                if name:
                    try: api.create_project(name, client, cost, sd.isoformat(), desc); st.success(f"'{name}' created!"); st.rerun()
    ...
    for proj in projects:
        ...
        pct = int(done / len(STAGES) * 100)
        with st.expander(f"**{proj['name']}** — {proj.get('client_name','')} | {pct}% | ₹{proj.get('total_cost',0):,.2f}"):
            st.progress(pct / 100)
            ...
            if st.button("💾 Save Stages", key=f"sv_{proj['id']}"):
                try: api.update_project(proj["id"], {"stages": new_stages}); st.success("Updated!"); st.rerun()
                except Exception as e: st.error(f"Error: {e}")

def page_log_time(user):
    st.markdown("### ⏱ Log Time")
    try: projects = api.list_projects(); employees = api.list_employees()
    except Exception as e: st.error(f"Error: {e}"); return
    if not projects: st.warning("No projects found."); return
    ...
    with st.form("log_time"):
        ...
        with a:
            ...
            if emp_match:
                st.text_input("Employee", value=emp_match["name"], disabled=True)
                sel_emp_id = emp_match["id"]
            else:
                emp_opts = {e["name"]: e["id"] for e in employees}
                if emp_opts:
                    sel_emp_id = emp_opts[st.selectbox("Employee *", list(emp_opts.keys()))]
                else:
                    st.warning("No employees registered."); sel_emp_id = None
        with b:
            hours = st.number_input("Hours *", min_value=0.5, max_value=16.0, step=0.5, value=1.0)
            log_date = st.date_input("Date", value=date.today())
        comments = st.text_area("What did you work on? *", height=100)
        if st.form_submit_button("📝 Log Time", type="primary", use_container_width=True):
            if not sel_emp_id: st.error("No employee profile linked.")
            elif not comments.strip(): st.error("Describe your work.")
            else:
                try:
                    api.create_time_log(sel_emp_id, proj_opts[sel_proj], hours, log_date.isoformat(), comments)
                    st.success(f"✅ Logged {hours}h on {sel_proj}"); st.rerun()
                except Exception as e: st.error(f"Error: {e}")
...
def page_my_logs(user):
    ...
    emp_id = user.get("employee_id", "")
    try:
        logs = api.list_time_logs(employee_id=emp_id) if emp_id else api.list_time_logs()
    ...
    st.metric("Total Hours", f"{sum(l.get('hours',0) for l in logs):.1f}h")
    df = pd.DataFrame([{
        "Date": l.get("date",""), "Project": pm.get(l.get("project_id"),"?"),
        "Hours": l.get("hours",0), "Comments": l.get("comments",""),
    } for l in logs]).sort_values("Date", ascending=False).reset_index(drop=True)
